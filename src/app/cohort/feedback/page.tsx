...
export default async function CohortFeedbackPage({
  searchParams,
}: {
  ...
  const selected = selectedId
    ? allEntries.find((m) => m.id === selectedId)
    : null;
  ...
  for (const e of allEntries) {
    if (e.sessionId) {
      countsBySession.set(
        e.sessionId,
        (countsBySession.get(e.sessionId) ?? 0) + 1
      );
    } else {
      legacyCount++;
    }
                ...
                <ul className="divide-y divide-cream-muted">
                  {entries.map((entry) => {
                    ...
                    return (
                      <li key={entry.id}>
                        <Link
                          href={`/cohort/feedback?id=${entry.id}${
